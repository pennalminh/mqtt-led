...
class DisplayCommunicator extends EventEmitter {
  ...
  connect = () => {
    if (
      ...
      this.connectionState === ConnectionState.SETTING_UP
    ) {
  ...
  private init = async (): Promise<boolean> => {
    const [serviceRequestError, serviceRequest] = await AsyncTask(
      this.askServiceVersion()
    );
  ...
  responseListener = (data: Buffer) => {
    ...
    if (data.length !== responseLen) {
      if (responseLen > data.length) {
        ...
        logger.info(
          `Data chunked ${this.receivingPacketLength} ${this.receivedBytes}`
        );
    ...
    switch (tcpResponse.cmd) {
      ...
      case CmdType.kErrorAnswer:
        {
          ...
          logger.error(
            `Got error anwser ${
              SdkErrorCode[tcpResponse.data.readUInt16LE()]
            } ${tcpResponse.data.readUInt16LE()}`
          );
        }
        break;
      case CmdType.kFileStartAnswer:
        {
          ...
          if (req) {
            if (
              responseSizeBuffer.readUInt16LE() ||
              responseErrorBuffer.readUInt16LE()
            ) {
      ...
      default: {
        logger.error({
          ...
          cmd: tcpResponse.cmd.toString(16),
        });
      }
    }
  };
  ...
  constructSdkTckPacket = (data: object, guid?: string): Buffer => {
    ...
    const payload =
      '<?xml version="1.0" encoding="utf-8"?>' + builder.build(kSDKServiceAsk);

    const xmlStrLen = Buffer.byteLength(payload, "utf8");

    const buff = Buffer.alloc(12 + xmlStrLen);
    ...
    Buffer.from(payload, "utf8").copy(buff, 12);
    return buff;
  };

  //TODO: Change return type from any to something more suitable
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  sdkCmdGet = (cmd: string, timeout = 1000): Promise<any> =>
  ...
  transferFile = async (file: Buffer) => {
    ...
    for (let s = 0; s < fileSize; s += this.maxPacketSize) {
      const packetSize =
        s + this.maxPacketSize > fileSize ? fileSize - s : this.maxPacketSize;
      ...
      this.emit(
        "uploadProgress",
        Math.round(((s + packetSize) / fileSize) * 100)
      );
  ...
  static searchForDevices = (
    network: string,
    port: number,
    timeout = 10000
  ): Promise<Array<CandidateDevice>> =>
    new Promise((resolve, reject) => {
      ...
      discoverySocket.on("message", function (rawResponse, remote) {
        ...
        if (response.header === CmdType.kSearchDeviceAnswer) {
          const uniqueCheck = devices.findIndex(
            (device) => device.model === response.payload
          );
          if (uniqueCheck >= 0) return;

          devices.push({
            model: response.payload + response.serialNumber,
